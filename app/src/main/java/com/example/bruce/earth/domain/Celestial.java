...
//        this.yAngle=yAngle;
    ...
    private void initVertexData() { //初始化顶点坐标的方法
        ...
        for (int i = 0; i < vCount; i++) {
            //随机产生每个星星的xyz坐标
            double angleTempJD = Math.PI * 2 * Math.random();//经度
            double angleTempWD = Math.PI * (Math.random() - 0.5f);//维度
            float UNIT_SIZE = 10f;
            vertices[i * 3] = (float) (UNIT_SIZE * Math.cos(angleTempWD) * Math.cos(angleTempJD));//x
            vertices[i * 3 + 1] = (float) (UNIT_SIZE * Math.cos(angleTempWD) * Math.sin(angleTempJD));//y
            vertices[i * 3 + 2] = (float) (UNIT_SIZE * Math.sin(angleTempWD));//z
        }
        //创建顶点坐标数据缓冲
