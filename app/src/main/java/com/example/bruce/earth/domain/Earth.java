...
import com.example.bruce.earth.MySurfaceView;
...
public class Earth {
    ...
    private int maSunLightLocationHandle;//光源位置属性引用id
    private int uDayTexHandle;//白天纹理属性引用id
    private int uNightTexHandle;//黑夜纹理属性引用id
    ...
    private void initShader(MySurfaceView mv) { //初始化着色器
        //加载顶点着色器的脚本内容
        String mVertexShader = ShaderUtil.loadFromAssetsFile("vertex_earth.sh", mv.getResources());
        ShaderUtil.checkGlError("==ss==");
        //加载片元着色器的脚本内容
        String mFragmentShader = ShaderUtil.loadFromAssetsFile("frag_earth.sh", mv.getResources());
        //基于顶点着色器与片元着色器创建程序
        ShaderUtil.checkGlError("==ss==");
